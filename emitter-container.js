(function() {
    var requestAnimationFrame = window.requestAnimationFrame ||
                                window.mozRequestAnimationFrame ||
                                window.webkitRequestAnimationFrame ||
                                window.msRequestAnimationFrame;

    window.requestAnimationFrame = requestAnimationFrame;

    /**
     * Defaults settings for emitter
     * @type {Object}
     */
    var defaults = function () {
        return {
            spawnInterval: 40,
            maxLifeTime: 5000,
            startSize: 30,
            endSize: 70,
            positionX: 500,
            positionY: 800,
            velocityX: 5,
            velocityY: -2,
            rotationSpeed: 180,
            startOpacity: 1,
            opacityDeclineSpeed: 0.01,
            paused: false,
            image: 'data:image/jpeg;base64,'
        };
    };

    function deepExtend (destination, source) {
        for (var property in source) {
            if (source[property] && source[property].constructor &&
                source[property].constructor === Object) {
                destination[property] = destination[property] || {};
                arguments.callee(destination[property], source[property]);
            } else {
                destination[property] = source[property];
            }
        }
        return destination;
    }

    function Particle(config) {

        this.asset = config.asset;

        this.positionX = config.positionX;
        this.positionY = config.positionY;

        this.size = 1;
        this.startSize = config.startSize;
        this.endSize = config.endSize;

        this.lifeTime = 1;
        this.startLife = new Date().getTime();
        this.maxLifeTime = config.maxLifeTime;

        this.angle = Math.random() * 359;
        this.rotationSpeed = config.rotationSpeed;

        this.startOpacity = config.startOpacity;
        this.opacityDeclineSpeed = config.opacityDeclineSpeed;

        this.velocityY = config.velocityY - (Math.random() * 0.5);
        this.velocityX = Math.floor(Math.random() * (-6) + 3) / config.velocityX;
    }

    Particle.prototype.update = function() {

        this.lifeTime = new Date().getTime() - this.startLife;
        this.angle += 0.2;

        var lifePerc = ((this.lifeTime / this.maxLifeTime) * 100);

        // Calculate size
        this.size = this.startSize + ((this.endSize - this.startSize) * lifePerc * 0.1);

        // Update opacity
        this.alpha = this.startOpacity - (lifePerc * this.opacityDeclineSpeed);
        this.alpha = Math.max(this.alpha, 0);

        // Update position
        this.positionX += this.velocityX;
        this.positionY += this.velocityY;
    };

    function Emitter(element, config) {

        /**
         * Settings
         * @type {Object}
         */
        this.settings = deepExtend(defaults(), config);

        /**
         * Reference to the last time the emitter was emitted
         * @type {Date}
         */
        this.lastTime = new Date().getTime();

        /**
         * Minimum time
         * @type {[type]}
         */
        this.spawnInterval = this.settings.spawnInterval;

        /**
         * Reference to the rendering canvas
         * @type {HTML element}
         */
        this.canvas = this.getCanvas();

        /**
         * Reference to the rendering context
         * @type {Object}
         */
        this.ctx = this.canvas.getContext('2d');

        /**
         * Holds the current particles spawn by the emitter
         * @type {Array}
         */
        this.particles = [];

        this.settings.maxLifeTime = Math.min(this.settings.maxLifeTime, (this.canvas.height / (1.5 * 60) * 1000));

        this.loadAsset(this.settings.image, function (asset) {
            this.settings.asset = asset;
            this.tick(this.settings);
        }.bind(this));

        window.onresize = this.onResize.bind(this);
    }

    Emitter.prototype = {
        spawn: function (config) {
            var now = new Date().getTime();

            if (!this.settings.paused && now > this.lastTime + this.spawnInterval) {
                var particle = new Particle(config);
                this.particles.push(particle);

                this.lastTime = now;
            }
        },

        tick: function (config) {
            var length = this.particles.length;

            this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);

            while (length--) {
                var particle = this.particles[length];

                if (this.isDeadParticle(particle)) {
                    // kill particle if it's life span is over
                    this.particles.splice(length, 1);
                } else {
                    particle.update();
                    this.renderParticle(particle);
                }
            }
            this.spawn(config);
            requestAnimationFrame(function () {
                this.tick(config);
            }.bind(this));
        },

        renderParticle: function (particle) {
            this.ctx.save();

            var offsetX = offsetY = -particle.size / 2;

            // Set position
            this.ctx.translate(particle.positionX - offsetX, particle.positionY - offsetY);

            // Set rotation
            this.ctx.rotate(particle.angle / particle.rotationSpeed * Math.PI);

            // Set opacity
            this.ctx.globalAlpha = particle.alpha;

            // Draw particle
            this.ctx.drawImage(particle.asset, offsetX, offsetY, particle.size, particle.size);

            this.ctx.restore();
        },

        isDeadParticle: function (particle) {
            return particle.positionY < 0 || particle.lifeTime > particle.maxLifeTime;
        },

        onResize: function () {
            this.canvas.width = document.body.scrollWidth;
            this.canvas.height = document.body.scrollHeight;
        },

        getCanvas: function () {
            var canvas = document.createElement('canvas');
            document.body.appendChild(canvas);
            canvas.style.position = 'absolute';
            canvas.style.zIndex = 0;
            canvas.style.top = 0;
            canvas.style.left = 0;
            canvas.style.right = 0;
            canvas.style.bottom = 0;
            canvas.width = document.body.scrollWidth;
            canvas.height = document.body.scrollHeight;
            return canvas;
        },

        /**
         * Load asset
         * @param  {String}   src URL or Base64
         * @param  {Function} cb  fired when image is ready
         */
        loadAsset: function (src, cb) {
            var image = new Image();

            image.src = src;

            image.onload = function () {
                cb(image);
            };
        }
    };

    window.Emitter = Emitter;

})();
